        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50">
              <th className="text-left py-2 px-4 font-medium text-gray-600 w-1/4">Faculty name</th>
              {DAYS.map(day => (
                <th key={day} className="text-center py-2 px-4 font-medium text-gray-600 w-1/6">{day}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                ...
                {DAYS.map(day => (
                  <td key={day} className="py-4 px-4">
                    <div className="flex flex-col items-center gap-1">
                      {member.availability?.find(a => a.day === day)?.slots.map((slot, idx) => (
                        <span
                          key={idx}
