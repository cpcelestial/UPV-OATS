...
React.useEffect(() => {
  const fetchTimeSlots = async () => {
    if (!selectedFacultyId || !datecheck) {
      ...
      if (docSnap.exists() && docSnap.data()[datecheck]) {
        ...
        if (Array.isArray(timeSlotData)) {
          const availableSlots = timeSlotData
            .filter((slot: { available?: boolean; booked?: boolean }) => 
        ...
        console.log("No time slots found, generating new schedule for:", selectedFacultyId, datecheck);
        await saveScheduleForUser(new Date(datecheck), selectedFacultyId);
